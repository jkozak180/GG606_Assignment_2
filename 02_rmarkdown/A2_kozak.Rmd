...
set_cancensus_api_key('<CensusMapper_bae5c1c5a3d31a162c898f63dd424bb3>', overwrite=FALSE, install=TRUE)
set_cancensus_cache_path('<local cache path>', overwrite=FALSE, install=TRUE)  #set within local 
Sys.getenv("CM_API_KEY") #confirm API registery 
Sys.getenv("CM_CACHE_PATH") #confirm cache patch saved to environ
```
...
CA01_regions=list_census_regions("CA01") %>%
  ...
  select(region, name, level, pop, census_year)
#Year 2006
...
```{r}
get_census("CA16", regions=list(CMA="46602"),vectors=c("v_CA16_434","v_CA16_435"), level="CSD")
```
