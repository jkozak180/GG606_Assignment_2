...
1. After looking at all of this data, my intuition would be to first look at general changes in education throughout the region over the years. Do we see more and more people throughout time getting higher and higher education levels? While a great place to start, this task was sadly too great for the amateur little coder. The first mission is to look at the popularity of bachelor degrees over time, which might have some relation to all of these PhD's that are supposedly here. \
2. Do we see a corresponding increase in PhD's appearing throughout time as well? When does this change in PhD holders occur? Has there been an increasing amount of people with bachelor degrees being granted, then into PhD's? Or do we think that we have external visitors coming to complete their research? \
...
```{r 3.1.1: New data frames Q2, include=TRUE}
#We can use the long education vector lists to find vectors that are best suited to our conditions. However, we still need to grab the affiliated shape files, which I did not do when pulling the CSD or DA data... whomp whomp. 
...
CA06_phd=get_census(dataset='CA06', regions=list(CMA="35521"),
                     ...
                     level='DA', quiet=TRUE, geo_format='sf', labels='short')
#2011 donly OVER A BACHELOR degree 15+, 25-64 (do not know if Master's or PhD)
...
```{r 3.1.1: New data frames Q2 cnt, include=TRUE}
#2001 only has one category... so no need for lots of maths
...
```{r 3.1.1: Q2 CA01 Figure, include=FALSE}
...
print(CA01_phd_fig)
```
```{r 3.1.1: Q2 CA06 Figure, include=FALSE}
...
```{r 3.1.1: Q2 CA11 Figure, include=FALSE}
...
```{r 3.1.1: Q2 CA16 Figure, include=FALSE}
...
```{r 3.1.1: Q2 CA21 Figure, include=FALSE}
#`region_name` changes here from the actual alphabetic name to numeric codes...
...
```{r 3.1.1: Q2 Figure, include=TRUE}
#Figure 1: Change in proportion of bachelor degree holders from 2001-2021
CA_phd_01_21 <-
animation::saveGIF(
  expr = {
...
```{r 3.1.1: Q2 Prop. Change 1, include=TRUE}
...
```{r 3.1.1: Q2 Prop. Change 2, include=TRUE}
...
```{r 3.1.1: Q2 Prop. Combine, include=TRUE}
...
```{r 3.1.1: Proportion CA01-CA21 BSc and PhD Figure 1, include=FALSE}
all_bachphd_proportions_fig1=all_bachphd_proportions %>%
  mutate(deg_type=factor(deg_type, levels=c("phd", "bachelor"),
                         labels=c("PhD", "Bachelor"))) %>%
#Generate the plot:
  ggplot() +
  geom_col(aes(x=census_year, y=percentage, fill=deg_type)) +
  geom_text(aes(x=census_year, y=percentage, label=scales::percent(percentage/100)), 
...
#Plot aesthetics:
  theme_minimal() +
  scale_fill_manual(name="Degree Type", values=c("#56B4E9", "#CC79A7"), 
                    labels=c("PhD", "Bachelor")) +
  guides(fill=guide_legend(title.position="top", title.hjust=0.5, title.vjust=0, 
                           reverse=TRUE)) +
  theme(legend.position="bottom", legend.title=element_text(size=10, colour="black"), 
...
print(all_bachphd_proportions_fig1)
```
...
```{r 3.1.1: Proportion CA01-CA21 BSc and PhD Figure 2, include=FALSE}
#side note: careful with the 'percentage' vs. degree_sum and position="fill" here
all_bachphd_proportions_fig2=all_bachphd_proportions %>%
  mutate(deg_type=factor(deg_type, levels=c("phd", "bachelor"),
                         labels=c("PhD", "Bachelor"))) %>%
#Generate the plot:
  ggplot() +
  geom_col(aes(x=census_year, y=percentage, fill=deg_type), position="fill") +
  scale_y_continuous(expand = expansion(mult=0, add=0)) +
...
print(all_bachphd_proportions_fig2)
```
...
```{r Proportion CA01-CA21 BSc and PhD Figure 3, include=TRUE}
...
```{r 3.1.1: Q2 CA01 Figure A, include=TRUE}
CA01_phd_fig=CA01_bachelor %>%     #keep same data frame because same data...
  filter(region_name=="Kingston") %>%  
#Generate the plot:  
  ggplot() +
...
```{r 3.1.1: Q2 CA06 Figure A, include=TRUE}
CA06_phd_fig=CA06_phd %>%
  filter(region_name=="Kingston") %>%  
#Generate the plot:  
  ggplot() +
...
```{r 3.1.1: Q2 CA11 Figure A, include=TRUE}
CA11_phd_fig=CA11_phd %>%
  filter(region_name=="Kingston") %>%  
#Generate the plot:  
  ggplot() +
...
```{r 3.1.1: Q2 CA16 Figure A, include=TRUE}
CA16_phd_fig=CA16_phd %>%
  filter(region_name=="Kingston") %>%  
#Generate the plot:  
  ggplot() +
...
```{r 3.1.1: Q2 CA21 Figure A, include=TRUE}
#`region_name` changes here from the actual alphabetic name to numeric codes...
CA21_phd_fig=CA21_phd %>%
  filter(csd_uid=="3510010") %>%  
#Generate the plot:  
