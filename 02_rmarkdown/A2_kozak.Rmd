...
**MASTER'S SECTION**
```{r 3.1.1: New data frames Q1.5, include=TRUE}
#We can use the long education vector lists to find vectors that are best suited to our conditions. However, we still need to grab the affiliated shape files, which I did not do when pulling the CSD or DA data... whomp whomp. 
...
```{r 3.1.1: New data frames Q1.5 cnt, include=TRUE}
#2001 only has one category... so no need for lots of maths
...
```{r 3.1.1: Q1.5 CA01 Figure, include=FALSE}
...
```{r 3.1.1: Q1.5 CA06 Figure, include=FALSE}
...
```{r 3.1.1: Q1.5 CA11 Figure, include=FALSE}
...
```{r 3.1.1: Q1.5 CA16 Figure, include=FALSE}
...
```{r 3.1.1: Q1.5 CA21 Figure, include=FALSE}
#`region_name` changes here from the actual alphabetic name to numeric codes...
...
```{r 3.1.1: Q2 Prop. Change 1 BSc, include=TRUE}
...
```{r 3.1.1: Q2 Prop. Change 2 MSc, include=TRUE}
...
```{r 3.1.1: Q2 Prop. Change 3 PhD, include=TRUE}
...
all_degree_proportions_fig1=all_degree_proportions %>%
  filter(!(census_year==2001 & (deg_type=="phd" | deg_type=="master"))) %>%
  filter(!(census_year==2011 & (deg_type=="phd"))) %>%
  mutate(deg_type=case_when(
          census_year==2011 & (deg_type=="phd" | deg_type=="master")~ "phd_master",
          TRUE~ deg_type)) %>%
  mutate(deg_type=factor(deg_type, levels=c("phd", "master","phd_master", "bachelor"),
                         labels=c("PhD", "Master's", "Master's/Phd","Bachelor"))) %>%
#Generate the plot:
  ggplot() +
...
#Plot aesthetics:
  theme_minimal() +
  scale_fill_manual(name="Degree Type", values=c("#56B4E9","#009E73","#999999", 
                                                 "#CC79A7"), 
                    labels=c("PhD", "Master's","Master's/PhD","Bachelor")) +
...
print(all_degree_proportions_fig1)
```
...
all_degree_proportions_fig2=all_degree_proportions %>%
  filter(!(census_year==2001 & (deg_type=="phd" | deg_type=="master"))) %>%
  filter(!(census_year==2011 & (deg_type=="phd"))) %>%
  mutate(deg_type=case_when(
          census_year==2011 & (deg_type=="phd" | deg_type=="master")~ "phd_master",
...
print(all_degree_proportions_fig2)
```
...
```{r 3.1.1: Q1.5 CA01 Figure A, include=TRUE}
CA01_msc_fig=CA01_bachelor %>%     #keep same data frame because same data...
  filter(region_name=="Kingston") %>%  
#Generate the plot:  
  ggplot() +
...
```{r 3.1.1: Q1.5 CA06 Figure A, include=TRUE}
CA06_msc_fig=CA06_msc %>%
  filter(region_name=="Kingston") %>%  
#Generate the plot:  
  ggplot() +
...
```{r 3.1.1: Q1.5 CA11 Figure A, include=TRUE}
CA11_msc_fig=CA11_msc %>%
  filter(region_name=="Kingston") %>%  
#Generate the plot:  
  ggplot() +
...
```{r 3.1.1: Q1.5 CA16 Figure A, include=TRUE}
CA16_msc_fig=CA16_msc %>%
  filter(region_name=="Kingston") %>%  
#Generate the plot:  
  ggplot() +
...
```{r 3.1.1: Q1.5 CA21 Figure A, include=TRUE}
#`region_name` changes here from the actual alphabetic name to numeric codes...
CA21_msc_fig=CA21_msc %>%
  filter(csd_uid=="3510010") %>%  
#Generate the plot:  
